...
/**
 * Resolves the type of a raw value string.
...
public final class Values {
	 ...
	 * already-declared, initialized variable visible from the given scope.
	 *
	 ...
	 * @throws UndeclaredVariableException    if raw is not a literal and names no visible variable
	 * @throws UninitializedVariableException if raw names a variable that is not initialized
	 */
	public static Type typeOf(String raw, Scope scope)
		...
		if (BOOLEAN.matcher(raw).matches()) {
			return Type.BOOLEAN;
		}
		if (CHAR.matcher(raw).matches()) {
			return Type.CHAR;
		}
		if (STRING.matcher(raw).matches()) {
			return Type.STRING;
		}

		// not a literal -> must be a variable reference
		Variable variable = scope.resolve(raw);
